    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            if args.dry_run:
                break
    # return train_loss
    return loss_metrics, loss_diff_sum
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        loss_metrics, loss_diff_sum = train(args, model, device, train_loader, optimizer, epoch)
        writer.add_scalar("LR", args.lr, epoch)

        loss_train = np.mean(loss_metrics)
        writer.add_scalar("Loss/train", loss_train, epoch)
        
        writer.add_scalar("Loss/Cumulative Loss Difference", loss_diff_sum, epoch)
